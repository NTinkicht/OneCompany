...
def event(index: int, kind: str, payload: dict, *, actor: str = "worker", seconds: int | None = None) -> dict:
...
def assignment(lease_id: str = "L1", *, head: str = A, actor: str = "worker", seconds: int = 0) -> dict:
    return event(
        1,
        "ROLE_LEASE_ASSIGNED",
        {
...
def initial_lease() -> dict:
    return lease_lifecycle._lease_from_event(assignment(), assignment()["payload"], ttl_seconds=100)


class LeaseLifecycleTests(unittest.TestCase):
    def derive(self, events, *, now):
        with (
            patch.object(lease_lifecycle.ledger_lib, "derive", side_effect=base_view),
            patch.object(lease_lifecycle, "lifecycle_policy", side_effect=policy),
        ):
            return lease_lifecycle.derive_lifecycle(events, now=now, durable=False)
    ...
    def test_assignment_has_deterministic_issue_and_expiry_and_implicit_expiry_revokes_authority(self):
        ...
        self.assertEqual([item["id"] for item in live["active_leases"]], ["L1"])
        self.assertEqual(live["active_leases"][0]["issued_at"], "2026-01-01T00:00:00Z")
        self.assertEqual(live["active_leases"][0]["expires_at"], "2026-01-01T00:01:40Z")

        expired = self.derive([assignment()], now=instant(100))
        self.assertEqual(expired["active_leases"], [])
        self.assertEqual([item["id"] for item in expired["expired_leases"]], ["L1"])
        self.assertEqual(expired["lease_history"][0]["work_unit"], "WU-A")
        self.assertEqual(expired["material_authors"], ["worker"])

    def test_supervision_heartbeat_never_renews(self):
        heartbeat = event(2, "SUPERVISION_CHECK", {"lease_id": "L1"}, seconds=90)
        view = self.derive([assignment(), heartbeat], now=instant(101))
        self.assertEqual(view["active_leases"], [])
        self.assertEqual(view["expired_leases"][0]["expires_at"], "2026-01-01T00:01:40Z")
    ...
    def test_verified_progress_renews_monotonically(self):
        ...
        self.assertEqual([item["id"] for item in view["active_leases"]], ["L1"])
        renewed = view["active_leases"][0]
        self.assertEqual(renewed["last_progress_head"], B)
        self.assertEqual(renewed["expires_at"], "2026-01-01T00:03:10Z")
    ...
    def test_same_head_heartbeat_style_renewal_is_rejected(self):
        ...
        self.assertIn(
            ...
            {item["reason"] for item in view["lifecycle_rejected_claims"]},
        )
    ...
    def test_renew_before_reap_wins_when_progress_extended_expiry(self):
        ...
        view = self.derive([assignment(), renewal, reap], now=instant(150))
        self.assertEqual([item["id"] for item in view["active_leases"]], ["L1"])
        self.assertIn("reap_before_expiry", {item["reason"] for item in view["lifecycle_rejected_claims"]})
    ...
    def test_reap_before_late_renewal_wins_and_future_work_is_not_deadlocked(self):
        ...
        replacement = assignment("L2", head=C, actor="worker2", seconds=130)
        replacement["event_id"] = "e4"
        replacement["local_sequence"] = 4
        view = self.derive([assignment(), reap, late, replacement], now=instant(150))
        self.assertEqual([item["id"] for item in view["active_leases"]], ["L2"])
        self.assertEqual([item["id"] for item in view["reaped_leases"]], ["L1"])
        self.assertIn("renew_source_not_active", {item["reason"] for item in view["lifecycle_rejected_claims"]})
        self.assertEqual(set(view["material_authors"]), {"worker", "worker2"})
    ...
    def test_transfer_from_expired_source_cannot_resurrect_authority(self):
        transfer = event(
            ...
            "ROLE_LEASE_TRANSFERRED",
            {
                ...
                "pr": 7,
                "start_head": B,
                "planning_snapshot": assignment()["payload"]["planning_snapshot"],
            },
            actor="worker2",
            seconds=110,
        )
    ...
    def test_local_event_store_reconstructs_authority_without_state_cache(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(lease_lifecycle.ledger_lib, "ledger_enabled", return_value=False),
                patch.object(lease_lifecycle.ledger_lib, "derive", side_effect=base_view),
                patch.object(lease_lifecycle, "lifecycle_policy", side_effect=policy),
            ):
                lease_lifecycle.append_coordination_event(
                    "ROLE_LEASE_ASSIGNED", "worker", payload, now=instant(0), local_path=path
                )
                first = lease_lifecycle.coordination_view(now=instant(50), local_path=path)
                # Re-open from the event file. No state.json value participates.
                second = lease_lifecycle.coordination_view(now=instant(50), local_path=path)
            self.assertEqual(first["active_leases"], second["active_leases"])
