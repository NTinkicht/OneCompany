...
POLICY = {
    ...
    "principals": [
        {"login": "Owner", "actor_id": "human-owner", "authorities": ["root", "merge_execution", "protected_merge"]},
...
class PlatformIdentityTests(unittest.TestCase):
    ...
    def test_first_identity_bootstrap_is_repository_owner_only(self):
        def fake(path: str):
            if "/contents/.onecompany/identity.json?ref=" in path:
    ...
    def test_spoofed_actor_label_cannot_replace_authenticated_principal(self):
        ...
        with patch.object(platform_identity, "_gh_json", side_effect=fake):
            identity, errors = platform_identity.authorize_current_principal("o/r", "b" * 40, "protected_merge")
        self.assertIsNone(identity)
        self.assertTrue(any("unknown" in error for error in errors), errors)
        # There is intentionally no CLI actor argument in this authorization API.
    ...
    def test_review_identity_is_exact_commit_and_base_policy_mapped(self):
        ...
        with patch.object(platform_identity, "_gh_json", side_effect=fake):
            identity, errors = platform_identity.review_platform_identity("o/r", 7, 42, "c" * 40, "b" * 40)
        self.assertEqual(errors, [])
    ...
    def test_stale_review_identity_fails_closed(self):
        def fake(path: str):
            if path.endswith("/pulls/7/reviews/42"):
                return {"commit_id": "a" * 40, "state": "APPROVED", "user": {"login": "review-bot"}}, None
            raise AssertionError(path)
