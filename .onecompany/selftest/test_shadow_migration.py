...
class ShadowMigrationTests(unittest.TestCase):
    def test_active_writer_fixture_fails_closed_without_mutation_readiness(self):
        fixture = ROOT / ".onecompany" / "selftest" / "fixtures" / "active-blocked-shadow.json"
        if not fixture.exists():
            self.fail("generic active-writer regression fixture missing")
    ...
    def test_idle_fixture_preserves_c1_blockers_without_fake_live_state(self):
        ...
        if not fixture.exists():
            self.fail("generic idle regression fixture missing")
        report = shadow_migration.analyze_manifest(shadow_migration.load_manifest(fixture))
        self.assertFalse(report["mutation_ready"])
        self.assertTrue(report["shadow_only"])
        self.assertFalse(report["target_mutated"])
