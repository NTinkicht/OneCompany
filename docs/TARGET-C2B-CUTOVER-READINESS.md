...
Target-specific observations and approvals belong to the target's own repository or separately permissioned tenant store; none is shipped as a reusable product default.
...
## Per-target evidence boundary

Each installation discovers its own target repository, active writers,
deployment environment, security gates, recovery operator and exact snapshot.
Operational observations and decisions remain with that target. An observation
for one application never grants authority over another. Unknown scopes and
stale evidence remain blockers until reverified. The generic product contains
only synthetic, non-operational regression fixtures.
