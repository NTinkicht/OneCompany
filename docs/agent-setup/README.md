...
A subscription/account — human or AI — is **not** the same thing as a routable OneCompany actor.
...
2. **Connection/configuration** - the product/CLI/app/account is authenticated for this repository.
...
4. Configure a genuinely independent reviewer — this may initially be the human owner.
...
Do not set an actor to `enabled: true` merely because you own a subscription or GitHub account. Enable it only after the intended execution path has been configured and the capabilities you intend to route have evidence in `.onecompany/readiness.json`.
...
Example: the human owner wants to remain the L2 merge/review gate but not be routinely assigned implementation. Verify `code_review`/`merge_execution` only; leave `implementation` unverified.
