...
def copy_item(source: Path, target: Path, force: bool) -> None:
    if source.is_dir():
        for child in source.rglob("*"):
            if not child.is_dir(): copy_item(child, target / child.relative_to(source), force)
        return
...
def initialize_contracts(target: Path) -> None:
    ...
    for source_name, target_name in CONTRACTS.items():
        ...
        if destination.exists(): print(f"KEEP existing project contract {target_name}"); continue
        shutil.copy2(source_dir / source_name, destination); print(f"INIT project contract {target_name}")
...
def initialize_control_plane(target: Path, project_name: str, repository: str, default_branch: str) -> None:
    ...
    config.setdefault("project", {})["name"] = project_name; config["project"]["repository"] = repository; config["project"]["default_branch"] = default_branch
...
def main() -> int:
    ...
    parser.add_argument("--target", required=True); parser.add_argument("--repository"); parser.add_argument("--project-name")
    parser.add_argument("--default-branch"); parser.add_argument("--initialize-contracts", action="store_true"); parser.add_argument("--force", action="store_true")
    args = parser.parse_args()
    target = Path(args.target).resolve(); target.mkdir(parents=True, exist_ok=True)
    if (target / ".onecompany").exists():
        print("ERROR: target already contains .onecompany. Use docs/UPGRADING.md."); return 2
    repository = args.repository or infer_github_repo(target)
    if not repository or not re.fullmatch(r"[^/\s]+/[^/\s]+", repository):
        print("ERROR: cannot infer valid GitHub repository; pass --repository owner/name."); return 2
    project_name = args.project_name or target.name; default_branch = args.default_branch or infer_default_branch(target)
    try:
        for relative in COPY_PATHS:
            source = ROOT / relative
            if source.exists(): copy_item(source, target / relative, args.force)
