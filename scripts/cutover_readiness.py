...
def analyze_cutover(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not shadow["mutation_ready"]:
        findings.append(
            _finding(
                "SHADOW_READINESS_REQUIRED",
                "C2a shadow migration blockers must be resolved before cutover rehearsal",
            )
        )
    ...
    if active_mutators:
        findings.append(
            _finding(
                ...
                "mutation-capable incumbents are not quiesced: "
                + ", ".join(sorted(active_mutators)),
            )
        )
    ...
    if quiescence_missing:
        findings.append(
            _finding(
                ...
                "mutation-capable incumbents require quiescence evidence and timestamp: "
                + ", ".join(sorted(quiescence_missing)),
            )
        )
    ...
    if not _nonempty(writer_identity):
        findings.append(
            _finding(
                "PROPOSED_WRITER_IDENTITY_MISSING",
    ...
    if ownership_ok and _nonempty(writer_identity):
        ownership_ok = all(
            post_ownership.get(capability) == writer_identity
    ...
    if not ownership_ok:
        findings.append(
            _finding(
                ...
                "every reviewed mutation capability must map exactly once to the proposed OneCompany writer",
            )
        )
    ...
    rollback_ok = (
        rollback.get("onecompany_disable_first") is True
        and rollback.get("legacy_restore_requires_human") is True
        and rollback.get("restore_order_reviewed") is True
        and _nonempty(rollback.get("evidence_ref"))
    )
    if not rollback_ok:
        findings.append(
            _finding(
                "ROLLBACK_PLAN_INCOMPLETE",
                "rollback must disable OneCompany first, require human legacy restore, preserve reviewed order, and cite evidence",
            )
        )
    ...
    return {
        "schema_version": "1.0",
        "repository": shadow.get("repository"),
        "default_branch": shadow.get("default_branch"),
        "shadow_mutation_ready": shadow["mutation_ready"],
        "shadow_blocker_codes": shadow["blocker_codes"],
        "cutover_ready": ready,
        "target_mutated": False,
        "mutation_authorized": False,
        "authority_effects": [],
        "mutation_capabilities": sorted(mutation_capabilities),
        "findings": findings,
        "blocker_codes": sorted({item["code"] for item in blockers}),
        "next_action": (
            "open a separately reviewed target-specific cutover change; this readiness gate grants no mutation authority"
            if ready
            else "resolve blockers and reconcile again; keep the target repository unchanged"
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prove quiescent cutover readiness without mutating the target repository"
    )
