...
def reviewer_eligible(actor_id: str, material_authors: set[str]) -> tuple[bool, list[str]]:
    ...
    actor = next((item for item in actors.get("actors", []) if item.get("id") == actor_id), None); ready = next((item for item in readiness_doc.get("actors", []) if item.get("actor_id") == actor_id), None)
    reasons: list[str] = []
...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--reviewer", required=True); parser.add_argument("--sha", required=True); parser.add_argument("--verdict", required=True, choices=VERDICTS); parser.add_argument("--pr", type=int); parser.add_argument("--evidence", action="append", default=[]); parser.add_argument("--summary", default=""); args = parser.parse_args()
    ...
    if args.verdict == "PASS — MERGE_READY":
        ...
        if checks.returncode != 0: print("REFUSED: required PR checks are not all green"); return 2
        if not checks.stdout.strip(): print("REFUSED: no required PR checks were reported"); return 2
    ...
    state["current_pr"] = pr; state["current_pr_head"] = live_head; state["current_material_authors"] = sorted(material_authors); state["current_gate"] = gate; state["company_state"] = "MERGE_READY" if args.verdict == "PASS — MERGE_READY" else "REVIEW_BLOCKED"; save_json(CONTROL / "state.json", state)
