#!/usr/bin/env python3
"""Record a binding exact-head review gate for one PR/Work Unit stream."""
...
def main() -> int:
    ...
    if emergency_stop_active():
        print("REFUSED: emergency stop is active; do not publish binding gates during containment"); return 2
    ...
    if not repo or not pr:
        print("REFUSED: repository and explicit --pr are required when multiple streams are active"); return 2
    ...
    for item in state.setdefault("active_streams", []):
        if item.get("pr") == pr:
            item["head"] = live_head; item["material_authors"] = sorted(material_authors); item["gate"] = gate
            item["status"] = "MERGE_READY" if args.verdict == "PASS — MERGE_READY" else "REVIEW_BLOCKED"; matched = True
    if not matched:
        state.setdefault("active_streams", []).append({"work_unit": state.get("current_work_unit") or "UNKNOWN", "lease_id": "unreconciled", "actor": None, "branch": None, "pr": pr, "head": live_head, "material_authors": sorted(material_authors), "gate": gate, "status": "MERGE_READY" if args.verdict == "PASS — MERGE_READY" else "REVIEW_BLOCKED"})
