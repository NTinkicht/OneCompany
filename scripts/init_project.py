...
def reset_control_plane(project_name: str, repository: str, default_branch: str) -> None:
    ...
    save_json(CONTROL / "queue.json", {"$schema": "./schemas/queue.schema.json", "schema_version": "1.0", "work_units": []})
    save_json(CONTROL / "state.json", {
        "$schema": "./schemas/state.schema.json", "schema_version": "1.0", "generated_or_reconciled_at": None,
        "repository_head": None, "company_state": "INITIALIZING", "current_work_unit": None, "current_pr": None,
        "current_pr_head": None, "current_material_authors": [], "active_leases": [], "current_gate": None,
        "ready_work_count": 0, "open_blockers": [], "human_decision_required": False,
...
def main() -> int:
    ...
    if current_repo != SOURCE_REPOSITORY:
        print(f"REFUSED: this installation already identifies as {current_repo!r}; init is only for an untouched OneCompany template copy. Use docs/UPGRADING.md for an existing company.")
        return 2
    ...
    print("All workers, dispatch paths, ledger and supervisors remain disabled until individually configured and smoke-tested.")
    print("Next: python onecompany.py validate && python onecompany.py status")
    return 0
