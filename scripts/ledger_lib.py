...
def ledger_config() -> dict[str, Any]: return load_json(CONTROL / "ledger.json")
def ledger_enabled() -> bool: return bool(ledger_config().get("enabled"))

def _repo_and_issue() -> tuple[str, int]:
    config = load_json(CONTROL / "config.json"); ledger = ledger_config(); repo = github_repo_from_config(config); issue = ledger.get("issue_number")
    if not repo: raise RuntimeError("config.project.repository must be owner/name")
    if not isinstance(issue, int) or issue <= 0: raise RuntimeError("ledger.issue_number must be configured")
    return repo, issue

def _trusted_publishers() -> set[str]: return set(ledger_config().get("trusted_publisher_logins", []))
def _gh_json(args: list[str]) -> Any:
    if not command_exists("gh"): raise RuntimeError("gh CLI is required for durable ledger access")
    result = run(["gh", *args])
...
def list_events() -> list[dict[str, Any]]:
    repo, issue = _repo_and_issue(); ledger = ledger_config(); trusted = _trusted_publishers()
    if not trusted: raise RuntimeError("ledger has no trusted publisher logins")
    expected_version = int(ledger.get("event_format_version", 1)); accepted = set(ledger.get("accepted_event_types", []))
    raw = _gh_json(["api", "--paginate", "--slurp", f"repos/{repo}/issues/{issue}/comments?per_page=100"])
    pages = raw if isinstance(raw, list) else [raw]; comments: list[dict[str, Any]] = []
    for page in pages:
        if isinstance(page, list): comments.extend(item for item in page if isinstance(item, dict))
        elif isinstance(page, dict): comments.append(page)
    events: list[dict[str, Any]] = []
    for comment in comments:
        login = ((comment.get("user") or {}).get("login"))
        if login not in trusted: continue
        body = comment.get("body") or ""
        if MARKER not in body: continue
        match = EVENT_RE.search(body)
        if not match: raise RuntimeError(f"malformed trusted ledger event in comment {comment.get('id')}")
        ...
        try: event = json.loads(match.group(1))
        except json.JSONDecodeError as exc: raise RuntimeError(f"invalid JSON in trusted ledger comment {comment.get('id')}: {exc}") from exc
        if event.get("version") != expected_version: raise RuntimeError(f"unsupported ledger event version in comment {comment.get('id')}")
        if event.get("type") not in accepted: raise RuntimeError(f"unsupported ledger event type in comment {comment.get('id')}: {event.get('type')}")
        if not isinstance(event.get("event_id"), str) or not event.get("event_id"): raise RuntimeError(f"trusted ledger event missing event_id in comment {comment.get('id')}")
        if not isinstance(event.get("actor"), str) or not event.get("actor"): raise RuntimeError(f"trusted ledger event missing actor in comment {comment.get('id')}")
        if not isinstance(event.get("payload"), dict): raise RuntimeError(f"trusted ledger event payload must be object in comment {comment.get('id')}")
        event["github_comment_id"] = comment.get("id"); event["github_comment_url"] = comment.get("html_url"); event["github_publisher"] = login; event["github_created_at"] = comment.get("created_at")
        events.append(event)
...
def post_event(event_type: str, actor: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not ledger.get("enabled"): raise RuntimeError("durable ledger is disabled")
    if event_type not in set(ledger.get("accepted_event_types", [])): raise RuntimeError(f"unsupported ledger event type: {event_type}")
    trusted = _trusted_publishers()
    if not trusted: raise RuntimeError("ledger requires at least one trusted publisher login")
    repo, issue = _repo_and_issue()
    event = {"version": int(ledger.get("event_format_version", 1)), "event_id": str(uuid.uuid4()), "type": event_type, "actor": actor, "timestamp": dt.datetime.now(dt.timezone.utc).isoformat(), "payload": payload}
    ...
    if login not in trusted:
        ...
        if comment_id: run(["gh", "api", "--method", "DELETE", f"repos/{repo}/issues/comments/{comment_id}"])
        raise RuntimeError(f"ledger publisher {login!r} is not trusted; event rejected")
    event["github_comment_id"] = posted.get("id"); event["github_comment_url"] = posted.get("html_url"); event["github_publisher"] = login; event["github_created_at"] = posted.get("created_at")
    return event
...
def derive(events: list[dict[str, Any]], pr: int | None = None) -> dict[str, Any]:
    ...
    def add_lease(lease_id: str, actor: str | None, payload: dict[str, Any], event: dict[str, Any]) -> bool:
        ...
        if role == "implementation":
            ...
            if same_wu is not None:
                conflicts.append({"event_id": event.get("event_id"), "type": event.get("type"), "reason": "implementation_lease_already_active_for_wu", "work_unit": work_unit, "winner_lease_id": same_wu.get("id"), "rejected_lease_id": lease_id})
                return False
            limit = int(planning.get("parallel_execution", {}).get("max_concurrent_implementation_streams", 1) or 1)
            if len(existing) >= limit:
                conflicts.append({"event_id": event.get("event_id"), "type": event.get("type"), "reason": "implementation_wip_limit_reached", "limit": limit, "rejected_lease_id": lease_id})
                return False
            ...
            for item in existing:
                ...
                if conflict:
                    conflicts.append({"event_id": event.get("event_id"), "type": event.get("type"), "reason": "implementation_scope_conflict", "with_lease_id": item.get("id"), "rejected_lease_id": lease_id, "details": reasons})
                    return False
        event_pr = normalize_pr(payload.get("pr"))
        active[lease_id] = {
            "id": lease_id, "role": role, "actor": actor, "work_unit": payload.get("work_unit"),
            "branch": payload.get("branch"), "pr": event_pr, "start_head": payload.get("start_head"),
            "planning_snapshot": payload.get("planning_snapshot", {}), "status": "active", "event": event,
        }
    ...
    for event in events:
        ...
        if event_id in seen_event_ids:
            conflicts.append({"event_id": event_id, "type": event.get("type"), "reason": "duplicate_event_id_ignored"}); continue
        if event_id: seen_event_ids.add(event_id)
        event_type = event.get("type"); actor = event.get("actor"); payload = event.get("payload") or {}; event_pr = normalize_pr(payload.get("pr"))
        if event_pr is not None: authors_by_pr.setdefault(event_pr, set())
        if event_type == "ROLE_LEASE_ASSIGNED":
            if payload.get("lease_id"): add_lease(str(payload.get("lease_id")), actor, payload, event)
        elif event_type == "ROLE_LEASE_RELEASED":
            if payload.get("lease_id"): active.pop(str(payload.get("lease_id")), None)
            ...
            if not old or old.get("role") != "implementation":
                conflicts.append({"event_id": event.get("event_id"), "type": event_type, "reason": "transfer_source_not_active", "old_lease_id": old_id, "new_lease_id": new_id}); continue
