...
from onecompany_lib import CONTROL, autonomy_number, budget_allows, command_exists, github_repo_from_config, load_json, run, save_json
...
def merge_actor_eligible(actor_id: str) -> tuple[bool, list[str]]:
    ...
    if "merge_execution" not in actor.get("capabilities", []): reasons.append("merge_execution_not_declared")
    ...
    if ready is None: reasons.append("missing_readiness")
        ...
        if "merge_execution" not in ready.get("verified_capabilities", []): reasons.append("merge_execution_not_verified")
        if "merge_execution" in ready.get("temporarily_unavailable_capabilities", []): reasons.append("merge_execution_temporarily_unavailable")
        access=ready.get("repository_access", {})
        if not access.get("read"): reasons.append("repository_read_not_verified")
        if not access.get("merge"): reasons.append("merge_permission_not_verified")
    return not reasons,reasons


def main() -> int:
    parser=argparse.ArgumentParser(); parser.add_argument("--actor", required=True); parser.add_argument("--pr", type=int); parser.add_argument("--method", choices=["merge","squash","rebase"], default="squash"); args=parser.parse_args()
    if not command_exists("gh") or run(["gh","auth","status"]).returncode != 0: print("REFUSED: authenticated gh CLI is required"); return 2
    config=load_json(CONTROL/"config.json"); state=load_json(CONTROL/"state.json"); repo=github_repo_from_config(config); pr=args.pr or state.get("current_pr")
    ...
    gate=state.get("current_gate"); active=[]
    if ledger_enabled():
        try:
            view=derive(list_events(), pr); gate=view.get("current_gate"); active=view.get("active_leases", [])
    ...
    if not eligible: print(f"REFUSED: merge actor {args.actor} is not eligible: {','.join(reasons)}"); return 2
    ...
    state["last_merge"]={"pr":pr,"approved_head":approved_sha,"merge_sha":payload.get("sha"),"actor":args.actor,"method":args.method,"merged_at":now}; state["current_work_unit"]=None; state["current_pr"]=None; state["current_pr_head"]=None; state["current_material_authors"]=[]; state["current_gate"]=None; state["company_state"]="POST_MERGE_RECONCILE"; save_json(CONTROL/"state.json",state)
    print(f"MERGED PR #{pr}: {payload.get('sha')} (approved exact head {approved_sha})"); return 0
