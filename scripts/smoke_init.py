...
def main() -> int:
    try:
        with tempfile.TemporaryDirectory(prefix="onecompany-init-") as temp:
            ...
            run(["git", "remote", "add", "origin", "https://github.com/example/template-copy.git"], target)
            ...
            require(json.loads((target / ".onecompany" / "queue.json").read_text())["work_units"] == [], "init must clear queue")
            ...
            validate = run([sys.executable, "onecompany.py", "validate"], target)
            require(validate.returncode == 0, f"initialized target validate failed:\n{validate.stdout}\n{validate.stderr}")
