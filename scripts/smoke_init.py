...
def main() -> int:
    try:
        with tempfile.TemporaryDirectory(prefix="onecompany-init-") as temp:
            ...
            target.mkdir()
            for name in (".github", ".onecompany", "scripts", "patterns", "overlays"):
            ...
            run(
                [
                    ...
                    "add",
                    "origin",
                    "https://github.com/example/template-copy.git",
                ],
                target,
            )
            initialized = run(
            ...
            require(
                config["project"]["repository"] == "example/template-copy",
                "init did not replace source repository",
            )
            require(config["autonomy"]["level"] == "L1", "init must reset autonomy to L1")
            require(
                config["safety"]["emergency_stop"] is False,
                "init emergency-stop default mismatch",
            )
            ...
            require(
                json.loads((target / ".onecompany" / "ledger.json").read_text())[
                    "enabled"
                ]
                is False,
            ...
            for command in (
                ...
                require(
                    result.returncode == 0,
                    f"initialized target {' '.join(command)} failed:\n"
                    f"{result.stdout}\n{result.stderr}",
                )
            ...
            require(
                second.returncode != 0,
                "init must refuse an already initialized company",
            )

        print("OneCompany template-init smoke PASS")
        return 0
