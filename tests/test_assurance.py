...
REFERENCE = ROOT / "examples" / "assurance" / "WU900.json"
TEMPLATE = ROOT / "templates" / "assurance" / "WU-assurance.json"
...
class AssuranceTests(unittest.TestCase):
    ...
    def test_draft_template_can_be_incomplete(self):
    ...
    def test_multiple_obligations_fail(self):
        packet = self.packet()
        packet["requirements"][0]["statement"] = "The system shall deny deletion and shall emit an alert."
    ...
    def test_nonexistent_requirement_test_target_fails(self):
        ...
        for link in packet["traceability"]:
            if link["relationship"] == "requirement_to_test":
                link["target"] = "TEST-999"
        self.assert_fails_with(packet, "targets missing test TEST-999")

    def test_wrong_work_unit_target_fails(self):
        packet = self.packet()
        for link in packet["traceability"]:
            if link["relationship"] == "requirement_to_work_unit":
                link["target"] = "WU999"
